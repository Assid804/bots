...
const Showcase = () => {
  return (
    <section className="bg-black text-white px-4 sm:px-6 md:px-16 py-20">
      {/* Heading */}
      <h2 className="text-4xl sm:text-6xl md:text-8xl font-serif mb-16 text-center md:text-left md:ml-[-53px]">
          ...
          {[Image, Image3, Image5].map((img, i) => (
            <div key={i} className="overflow-hidden rounded-xl">
              <img
                src={img}
                alt=""
                className="w-full transition-transform duration-500 ease-out hover:scale-110 cursor-pointer"
              />
            </div>
          ))}
        </div>
